...
def install_ssh_components(platform, container)
  ...
  when %r{centos}, %r{^el-}, %r{eos}, %r{oracle}, %r{redhat}, %r{scientific}
    ...
    run_local_command("docker exec #{container} bash -exc 'rm -f /var/lib/rpm/__db*; db_verify /var/lib/rpm/Packages; rpm --rebuilddb; yum clean all; yum install -y sudo openssh-server openssh-clients'")
